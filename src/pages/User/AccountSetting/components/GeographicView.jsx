...
@connect(({ userAccountSetting, loading }) => {
  const { province, city } = userAccountSetting;
  return {
    province,
    city,
    loading: loading.models.userAccountSetting,
  };
})
class GeographicView extends Component {
  componentDidMount = () => {
    ...
    if (dispatch) {
      dispatch({
        type: 'userAccountSetting/fetchProvince',
      });
    }
  };
  ...
  componentDidUpdate(props) {
    ...
    if (!props.value && !!value && !!value.province) {
      if (dispatch) {
        dispatch({
          type: 'userAccountSetting/fetchCity',
